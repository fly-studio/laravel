...
use Addons\Entrust\Tests\Models\OwnableObject;
...
class UserTest extends TestCase
{
    ...
    public function testRolesRelationship()
    {
        ...
        |------------------------------------------------------------
        */
        $this->app['config']->set('entrust.use_teams', false);
    ...
    public function testHasRole()
    {
        ...
        */
        ...
        $this->app['config']->set('entrust.teams_strict_check', true);
        $this->assertFalse($this->user->hasRole('role_c'));
        $this->app['config']->set('entrust.teams_strict_check', false);
    ...
    public function testMagicCanPermissionMethod()
    {
        ...
        */
        ...
        |------------------------------------------------------------
        */
        $this->app['config']->set('entrust.magic_can_method_case', 'kebab_case');
        $this->assertTrue($this->user->canManageUser());

        $this->app['config']->set('entrust.magic_can_method_case', 'snake_case');
        $this->assertTrue($this->user->canManageUser());

        $this->app['config']->set('entrust.magic_can_method_case', 'camel_case');
        $this->assertTrue($this->user->canManageUser());
    }
    ...
    public function testAttachRole()
    {
        ...
        */
        ...
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->attachRole($role, $team->id));
        $this->assertEquals($team->id, $this->user->roles()->first()->pivot->team_id);
        $this->user->roles()->sync([]);
        // Can attach role by passing the role and team name
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->attachRole($role, 'team_a'));
        $this->assertEquals($team->id, $this->user->roles()->first()->pivot->team_id);
        $this->user->roles()->sync([]);
    ...
    public function testDetachRole()
    {
        ...
        */
        ...
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->detachRole('role_a'));
        ...
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->detachRole($role, $team));
        $this->assertEquals(0, $this->user->roles()->count());
        $this->user->roles()->attach($role->id, ['team_id' => $team->id]);
        // Can detach role by passing the role and team id
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->detachRole($role, $team->id));
        $this->assertEquals(0, $this->user->roles()->count());
        $this->user->roles()->attach($role->id, ['team_id' => $team->id]);
        // Can detach role by passing the role and team name
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->detachRole($role, 'team_a'));
    ...
    public function testAttachRoles()
    {
        ...
        */
        ...
        |------------------------------------------------------------
        */
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $user->attachRoles([1, 2, 3]));
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $user->attachRoles([1, 2, 3], 'TeamA'));
    }
    ...
    public function testDetachRoles()
    {
        ...
        */
        ...
        |------------------------------------------------------------
        */
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $user->detachRoles([1, 2, 3]));
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $user->detachRoles([]));
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $user->detachRoles([1, 2, 3], 'TeamA'));
    }
    ...
    public function testSyncRoles()
    {
        ...
        */
        ...
        |------------------------------------------------------------
        */
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->syncRoles($roles));
        $this->assertEquals(2, $this->user->roles()->count());
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->syncRoles($roles, 'team_a'));
        $this->assertEquals(4, $this->user->roles()->count());

        $this->app['config']->set('entrust.use_teams', false);
        $this->user->syncRoles([]);
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->syncRoles($roles, null));
        $this->assertEquals(2, $this->user->roles()->count());
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->syncRoles($roles, 'team_a', false));
        $this->assertEquals(2, $this->user->roles()->count());
    }
    ...
    public function testSyncRolesWithoutDetaching()
    {
        ...
        */
        ...
        |------------------------------------------------------------
        */
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->syncRolesWithoutDetaching($roles));
        $this->assertEquals(3, $this->user->roles()->count());
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->syncRolesWithoutDetaching($roles, 'team_a'));
        $this->assertEquals(5, $this->user->roles()->count());

        $this->app['config']->set('entrust.use_teams', false);
        $this->user->detachRoles([1]);
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->syncRolesWithoutDetaching($roles, null));
        $this->assertEquals(4, $this->user->roles()->count());
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->syncRolesWithoutDetaching($roles, 'team_a', false));
        $this->assertEquals(4, $this->user->roles()->count());
    }
    ...
    public function testAttachPermission()
    {
        ...
        */
        ...
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->attachPermission($permission, $team->id));
        ...
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->attachPermission($permission, 'team_a'));
        $this->assertEquals($team->id, $this->user->permissions()->first()->pivot->team_id);
        $this->user->permissions()->sync([]);
    ...
    public function testDetachPermission()
    {
        ...
        */
        ...
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->detachPermission('permission_a'));
        ...
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->detachPermission($permission, $team));
        $this->assertEquals(0, $this->user->permissions()->count());
        $this->user->permissions()->attach($permission->id, ['team_id' => $team->id]);
        // Can detach permission by passing the permission and team id
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->detachPermission($permission, $team->id));
        $this->assertEquals(0, $this->user->permissions()->count());
        $this->user->permissions()->attach($permission->id, ['team_id' => $team->id]);
        // Can detach permission by passing the permission and team name
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->detachPermission($permission, 'team_a'));
    ...
    public function testAttachPermissions()
    {
        ...
        */
        ...
        |------------------------------------------------------------
        */
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $user->attachPermissions([1, 2, 3]));
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $user->attachPermissions([1, 2, 3], 'TeamA'));
    }
    ...
    public function testDetachPermissions()
    {
        ...
        */
        ...
        |------------------------------------------------------------
        */
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $user->detachPermissions([1, 2, 3]));
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $user->detachPermissions([]));
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $user->detachPermissions([1, 2, 3], 'TeamA'));
    }
    ...
    public function syncPermissions()
    {
        ...
        */
        ...
        |------------------------------------------------------------
        */
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->syncPermissions($permissions));
        $this->assertEquals(2, $this->user->permissions()->count());
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->syncPermissions($permissions, 'team_a'));
        $this->assertEquals(4, $this->user->permissions()->count());

        $this->app['config']->set('entrust.use_teams', false);
        $this->user->syncPermissions([]);
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->syncPermissions($permissions, null));
        $this->assertEquals(2, $this->user->permissions()->count());
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->syncPermissions($permissions, 'team_a', false));
        $this->assertEquals(2, $this->user->permissions()->count());
    }
    ...
    public function testSyncPermissionsWithoutDetaching()
    {
        ...
        */
        ...
        |------------------------------------------------------------
        */
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->syncPermissionsWithoutDetaching($permissions));
        $this->assertEquals(3, $this->user->permissions()->count());
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->syncPermissionsWithoutDetaching($permissions, 'team_a'));
        $this->assertEquals(5, $this->user->permissions()->count());

        $this->app['config']->set('entrust.use_teams', false);
        $this->user->detachPermissions([1]);
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->syncPermissionsWithoutDetaching($permissions, null));
        $this->assertEquals(4, $this->user->permissions()->count());
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $this->user->syncPermissionsWithoutDetaching($permissions, 'team_a', false));
        $this->assertEquals(4, $this->user->permissions()->count());
    }
    ...
    protected function assertWasAttached($objectName, $result)
    {
        ...
        $this->assertInstanceOf('Addons\Entrust\Tests\Models\User', $result);
