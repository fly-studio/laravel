...
trait UserTrait
{
    ...
    {
        $cacheKey = 'entrust_permissions_for_user_' . $this->getKey();
    ...
    public function roles()
    {
        ...
        if (Config::get('entrust.use_teams')) {
            $roles->withPivot(Config::get('entrust.foreign_keys.team'));
        }
    ...
    public function rolesTeams()
    {
        ...
        return $this->morphToMany(
                Config::get('entrust.models.team'),
                'user',
                Config::get('entrust.tables.role_user'),
                Config::get('entrust.foreign_keys.user'),
                Config::get('entrust.foreign_keys.team')
            )
            ->withPivot(Config::get('entrust.foreign_keys.role'));
    }
    ...
    public function permissions()
    {
        ...
        if (Config::get('entrust.use_teams')) {
            $permissions->withPivot(Config::get('entrust.foreign_keys.team'));
        }
    ...
    public function hasPermission($permission, $team = null, $requireAll = false)
    {
        ...
        foreach ($this->cachedRoles() as $role) {
            $role = Helper::hidrateModel(Config::get('entrust.models.role'), $role);
    ...
    private function attachModel($relationship, $object, $team)
    {
        ...
        if (Config::get('entrust.use_teams')) {
            ...
            if (
                    ...
                    ->wherePivot(Config::get("entrust.foreign_keys.{$objectType}"), $object)
                    ->count()
                ) {
                return $this;
            }
        ...
        $this->flushCache();
        $this->fireEntrustEvent("{$objectType}.attached", [$this, $object, $team]);
    ...
    private function detachModel($relationship, $object, $team)
    {
        ...
        $this->flushCache();
        $this->fireEntrustEvent("{$objectType}.detached", [$this, $object, $team]);
    ...
    private function syncModels($relationship, $objects, $team, $detaching)
    {
        ...
        $objectType = Str::singular($relationship);
        $mappedObjects = [];
        $useTeams = Config::get('entrust.use_teams');
        ...
        $this->flushCache();
        $this->fireEntrustEvent("{$objectType}.synced", [$this, $result, $team]);
    ...
    {
        if ($thing instanceof \Addons\Entrust\Contracts\Ownable) {
    ...
    public function flushCache()
    {
        ...
        Cache::forget('entrust_permissions_for_user_' . $this->getKey());
    }
    ...
    {
        $case = str_replace('_case', '', Config::get('entrust.magic_can_method_case'));
