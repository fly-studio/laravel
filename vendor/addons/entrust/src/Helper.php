...
class Helper
{
    ...
    public static function getIdFor($object, $type)
    {
        if (is_null($object)) {
            ...
            return call_user_func_array([
                Config::get("entrust.models.{$type}"), 'where'
    ...
    {
        return Config::get('entrust.foreign_keys.team');
    }
    ...
    {
        if (
            !Config::get('entrust.use_teams')
            || (!Config::get('entrust.teams_strict_check') && is_null($team))
        ) {
            return true;
        }
